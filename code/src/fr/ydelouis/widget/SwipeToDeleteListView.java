...
public class SwipeToDeleteListView extends ListView {
	...
	private void init() {
		...
		if (selector == null)
			selector = getResources().getDrawable(
	...
	private void setAlpha(Canvas canvas, ItemState itemState) {
		float alphaPercentage = 1f - 2f * Math.abs(itemState
				.getDragPercentage());
	...
	private boolean isToBeDeleted() {
		...
		if (itemState == null || itemState.getState() != State.Dragged)
			return false;
		float dragPercentage = itemState.getDragPercentage();
		return Math.abs(dragPercentage) > 0.35;
	}
	...
	private void startDeletionConfirmedAnimation(ItemState itemState,
		...
		new DeletionConfirmedAnimation(this, itemState, animLength, isLeft)
				.start();
	}
	...
	void onItemDeleted(ItemState itemState, boolean isLeft) {
		if (isConfirmNeeded) {
			if (onItemDeletedListener != null)
				onItemDeletedListener.onItemDeleted(this,
						itemState.getPosition(), isLeft);
			itemState.setState(ItemState.State.Deleted);
			notifyDataSetChanged();
		} else {
			startDeletionConfirmedAnimation(itemState, isLeft);
			invalidate();
		}
		mIsLeft = isLeft;
	}

	void onItemDeletionConfirmed(ItemState itemState, boolean isLeft) {
		if (onItemDeletedListener != null) {
			if (isConfirmNeeded)
				onItemDeletionConfirmedListener.onItemDeletionConfirmed(this,
						itemState.getPosition());
			else
				onItemDeletedListener.onItemDeleted(this,
						itemState.getPosition(), isLeft);
		}
	...
	public void confirmDeletion(int position) {
		...
		if (itemState == null)
			return;
		startDeletionConfirmedAnimation(itemState, mIsLeft);
	}
	...
	public void setOnItemDeletedListener(
	...
	public void setOnItemDeletionConfirmedListener(
			OnItemDeletionConfirmedListener onItemDeletionConfirmedListener) {
